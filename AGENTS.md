...
| Path | Purpose |
|------|---------|
| `lib/main.dart` | Entry: `SplashScreen` → `HomePage` |
...
| `lib/screens/ayah_list_page.dart` | Surah list → navigates to `MushafScreen(pageNumber)` |
...
## Assets
- `assets/quran_data/pages/` — 604 JSON files (one per page, word-level data)
- `assets/quran_data/verses.json` — verse lookup data
- `assets/quran_data/fonts/` — 47 QCF Hafs .ttf fonts (page-range mapped via `font-map.json`)
- `assets/images/` — background.png, logo.jpg, surah header frames, ayah number frames
...
- Reciter data fetched live from `https://api.qurancdn.com/api/qdc/audio/reciters` — no local list
